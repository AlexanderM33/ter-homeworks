...
Не приведен второй лейбл для **resource "docker_image"** нужно дать ему название
Некорректное имя - терраформ в логи пишет, что оно должно начинаться с буквы или подчеркивания и может содержать только буквы, цифры, подчеркивания и тире resource "docker_container" "nginx"
опечатка в docker_image.nignx.image_id - docker_image.ngnix.image_id
опечатка name  = "example_${random_password.random_string_FAKE.resulT}" - name  = "example_${random_password.random_string.result}"
***
